...
void lidar_cb(sensor_msgs::LaserScan msg){
    ...
    for(i = 0; i<len-1; i++){
        if(std::isinf(range[i])) range[i] = 4.2;
    }
    ...
    for(i=0; i<len-1; i++){
        if(i != len-2){
            if((range[i+1]-range[i]) <= -0.7) pole_wall.push_back(i+1);
            else if((range[i+1]-range[i]) >=0.7) pole_wall.push_back(i);
            else;
        }
        else{
            if((range[len-2]-range[0]) <= -0.7) pole_wall.push_back(len-2);
            else if((range[len-2]-range[0]) >= 0.7) pole_wall.insert(pole_wall.begin(), 0);
            else;
        }
    }
    ...
    while(i < len1-1){
        if(i != 0){
        ...
        else{ // first index of pole_wall[]
            if(pole_wall[i] == 0){ // when the first index is 0 degrees
                // when pole_wall[len1-1] ~ pole_wall[0] is a pole
                if(range[pole_wall[len1-1]-1]>range[pole_wall[len1-1]] && range[pole_wall[i]]<range[pole_wall[i]]+1){
            ...
            else{ // when the first index is not 0 degrees
                if(range[pole_wall[len1-1]-1]>range[pole_wall[len1-1]] && range[pole_wall[i]]<range[pole_wall[i]]+1){
                    new_polewall.push_back(pole_wall[len1-1]);
                    new_polewall.push_back(pole_wall[0]);
                    i++;
    ...
    for(i=0; i < last_polewall.size(); i+=2){
        if(i == 0 && last_polewall[i]>340){ // first idx is over 340
            pole_idx = (last_polewall[i+1]-last_polewall[i] + 360)/2 + last_polewall[i];

            if(pole_idx > 358) pole_idx -= 358;
